...
export const UpdateSection = async (
  ...
  try {
    ...
    if (
      section.course.creatorId !== req.user?.id &&
      req.user?.role !== "ADMIN"
    ) {
      return res.status(403).json({ message: "Access denied" });
    }
...
export const CreateNewLesson = async (
  ...
  try {
    const { sectionId, title, description, videoUrl, duration, order } =
      req.body;
    ...
    if (
      section.course.creatorId !== req.body.user?.id &&
      req.body.user?.role !== "ADMIN"
    ) {
      return res.status(403).json({ message: "Access denied" });
    }
...
export const UpdateALesson = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    ...
    // Check if user is the course creator
    if (
      lesson.section.course.creatorId !== req.user?.id &&
      req.user?.role !== "ADMIN"
    ) {
      return res.status(403).json({ message: "Access denied" });
    }
...
export const DeleteALesson = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    ...
    // Check if user is the course creator
    if (
      lesson.section.course.creatorId !== req.body.user?.id &&
      req.body.user?.role !== "ADMIN"
    ) {
      return res.status(403).json({ message: "Access denied" });
    }
...
export const FetchEnrolledLesson = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    ...
    if (!isCreator && req.body.user?.role !== "ADMIN") {
