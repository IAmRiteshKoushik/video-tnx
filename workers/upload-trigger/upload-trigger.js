...
export default async function (context, myBlob) {
  ...
  try {
    ...
    const encodedMessage = Buffer.from(JSON.stringify(queueMessage)).toString(
      "base64",
    );
    ...
    context.log(
      `Message added to queue 1 (${queueName1}): ${JSON.stringify(queueMessage)}`,
    );
    await queueClient2.sendMessage(encodedMessage);
    context.log(
      `Message added to queue 2 (${queueName2}): ${JSON.stringify(queueMessage)}`,
    );
